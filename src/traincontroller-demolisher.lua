...
function Traincontroller.Demolisher:initGlobalData()
  local Demolisher = {
    ...
    ["demolisherStates"] = { -- states in the builder process
      ...
      ["idle"        ] = 1, -- wait until a depot request a train
...
function Traincontroller.Demolisher:updateController(surfaceIndex, position)
  ...
  if controllerStatus == controllerStates["dispatching"] then
    ...
    if self:canBuildNextTrain(trainDemolisherIndex, controllerEntity) then
      -- if we can build a new train, we move on to the next step
      --game.print("Start building a train of length: "..#Trainassembly:gettrainDemolisher(trainDemolisherIndex))
  ...
  if controllerStatus == controllerStates["building"] then
    ...
    if self:buildNextTrain(trainDemolisherIndex) then
      -- if the whole train is build, we can send it away
      --game.print("Finished building a train of length: "..#Trainassembly:gettrainDemolisher(trainDemolisherIndex))
  ...
  if controllerStatus == controllerStates["idle"] then
    ...
    --  --game.print("Ready to dispatch a train of length: "..#Trainassembly:gettrainDemolisher(trainDemolisherIndex))
  ...
  if controllerStatus == controllerStates["dispatch"] then
    ...
    if self:assembleNextTrain(trainDemolisherIndex, controllerEntity.backer_name) then
      ...
      --game.print("Leaving train of length: "..#Trainassembly:gettrainDemolisher(trainDemolisherIndex))
...
function Traincontroller.Demolisher:canBuildNextTrain(trainDemolisherIndex, trainDemolisherController)
  -- We need to check for each builder if it can place a train there
  local trainDemolisher = Trainassembly:gettrainDemolisher(trainDemolisherIndex)
  if not trainDemolisher then return false end
  --game.print("checking if it can build a train with length: "..#trainDemolisher)
  ...
  local signalEntity = Traincontroller:getTrainHiddenEntity(trainDemolisherController, 2)
  ...
  signalEntity = Traincontroller:getTrainHiddenEntity(trainDemolisherController, 1)
  if signalEntity.signal_state ~= defines.signal_state.open then return false end

  --game.print("signal was green!")

  -- STEP 2: Check if each building can place the train
  for _, builderLocation in pairs(trainDemolisher) do
...
function Traincontroller.Demolisher:buildNextTrain(trainDemolisherIndex)
...
function Traincontroller.Demolisher:depotIsRequestingTrain(controllerEntity)
...
function Traincontroller.Demolisher:assembleNextTrain(trainDemolisherIndex, depotName)
  -- The whole train is assembled, so now we can send it away

  -- STEP 1: Get the train that is created (not the individual carriages)
  local train = self:getDemolishingCarriages(trainDemolisherIndex)
  ...
  for _, builderLocation in pairs(Trainassembly:gettrainDemolisher(trainDemolisherIndex)) do
...
function Traincontroller.Demolisher:deactivateOnTick()
  -- TODO: check if it can be deactivated
